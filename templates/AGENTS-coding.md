...
- **Fit tool to task.** Use an MCP tool when it is the purpose-built path (e.g. browser MCP for UI verification, search MCP for time-sensitive retrieval). Prefer workspace tools for repo-local work (read/edit, grep, tests) unless an MCP is clearly the correct integration for that system.
...
- **Scan for relevance early.** For non-trivial tasks, check whether an available skill matches the user’s goal (workflow conventions, repo-specific change process, testing or review gates). If one matches, **read that skill and follow it** rather than improvising a parallel process.
...
*These directives encode behavioral patterns for accurate, safe, and efficient agentic coding. They are LLM-agnostic and IDE-agnostic. Sections on MCP tools and agent skills apply when the host environment exposes those capabilities.*
